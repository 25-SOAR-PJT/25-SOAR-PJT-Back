...
import org.project.soar.model.youthpolicy.dto.YouthPolicyPopularView;
...
public interface YouthPolicyBookmarkRepository extends JpaRepository<YouthPolicyBookmark, Long> {
    ...
    // YouthPolicyBookmarkRepository
    @Query("""
                SELECT b.policy.policyId
                FROM YouthPolicyBookmark b
                WHERE b.user = :user AND b.policy.policyId IN :policyIds
            """)
    List<String> findBookmarkedPolicyIds(@Param("user") User user, @Param("policyIds") List<String> policyIds);

    List<YouthPolicyBookmark> findAllByUserAndPolicyPolicyIdIn(User user, List<String> policyIds);
    ...
    ORDER BY p.businessPeriodEnd ASC
""")
    List<YouthPolicy> findLatestBookmarkByEndDate(@Param("user") User user, Pageable pageable);

    // 전체 인기 (북마크 수 높은 순) — Pageable로 Top N
    @Query("""
                SELECT p.policyId AS policyId, p.policyName AS policyName, p.largeClassification As largeClassification
                FROM YouthPolicyBookmark b
                JOIN b.policy p
                GROUP BY p
                ORDER BY COUNT(b) DESC
            """)
    List<YouthPolicyPopularView> findPopularByBookmarksName(Pageable pageable);
